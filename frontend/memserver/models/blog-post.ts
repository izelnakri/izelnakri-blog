...
export default class BlogPost extends Model {
  static defaultAttributes = {
    slug() {
      debugger;
      return this && this.title ? dasherize(this.title) : null;
    },
    meta_title() {
      return this.title;
    },
  ...
  static tags(blogPost) {
    return BlogPostTag.findAll({ blog_post_id: blogPost.id });
  }

  static serializer(modelOrArray) {
    if (Array.isArray(modelOrArray)) {
      return modelOrArray.map((model) =>
        Object.assign({}, this.serialize(model), {
          tags: this.tags(model),
        })
      );
    }

    return Object.assign({}, this.serialize(modelOrArray), {
      tags: this.tags(modelOrArray),
    });
  }
}
