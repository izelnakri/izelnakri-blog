...
import { InternalModel } from 'memserver/model'; // eslint-disable-line

const { BlogPost, Comment, Email, User } = Models;
...
export default function () {
  ...
  this.post('/login', ({ params }) => {
    ...
    const user = User.find(email.user_id) as InternalModel;
  ...
  this.get('/blog-posts', ({ queryParams }) => {
    // NOTE: HANDLE unpublished comments
  ...
  this.put('/blog-posts/:id', ({ headers, params }) => {
    ...
    const blogPost = BlogPost.find(params.id) as InternalModel;
    ...
    const updatedBlogPost = BlogPost.update(
      Object.assign({ id: params.id }, blogPostParams, {
  ...
  this.post('/comments', ({ headers, params }) => {
    // NOTE: create Email if not exists
    const user = User.findFromHeaders(headers);
